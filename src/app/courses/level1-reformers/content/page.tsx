...
export default function Level1ReformersContent() {
  const router = useRouter();

  // DEBUG: Plain HTML link for troubleshooting
  // Place this outside the React component
  if (typeof window !== 'undefined') {
    const debugLink = document.createElement('a');
    debugLink.href = '/courses/anatomy-xformer';
    debugLink.innerText = 'DEBUG: Go to Anatomy of the Xformer';
    debugLink.style.position = 'fixed';
    debugLink.style.bottom = '10px';
    debugLink.style.left = '10px';
    debugLink.style.background = 'red';
    debugLink.style.color = 'white';
    debugLink.style.padding = '10px 20px';
    debugLink.style.zIndex = '9999';
    debugLink.style.fontSize = '20px';
    debugLink.target = '_self';
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="fixed top-4 left-4 z-50">
        <button
          onClick={() => router.back()}
          className="flex items-center space-x-2 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg hover:bg-white transition-all duration-200"
        >
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        {/* Chapter 1: The Anatomy of the Xformer */}
        <div className="mb-8">
          <Link href="/courses/anatomy-xformer" target="_self">
            <span className="block font-medium text-gray-900 text-sm hover:underline bg-yellow-200 border border-yellow-600 rounded px-4 py-2 cursor-pointer">
              The Anatomy of the Xformer (Click Me)
            </span>
          </Link>
        </div>
        <motion.div 
          ...
          className="bg-white rounded-lg shadow-lg p-8 mb-8"
        >
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Chapter 1: The Anatomy of the Xformer</h2>
        </motion.div>
